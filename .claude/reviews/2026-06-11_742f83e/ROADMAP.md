...
| E-A-3 + L-D-2 | 🟡 | 🙋 | インスタンス名変更で旧ボリューム孤立・無警告（前回から継続）。加えて manage.ts:100 の `instanceName` が setup 後に stale（ヘッダー・`ssh <旧名>`・injectSshConfig が旧名参照）。setup ケース末尾での再読込 + 変更検出時に旧ボリューム一覧と削除案内 | 2-3時間 | 次の CLI 作業時 |
| G: 鍵分離 | 機能 | 🙋 | keys.json のインスタンス分離（前回から継続）。store/束を `~/.ssh/cloopy/instances/<名前>/` へ。今回確認: `authorizedKeysPath()` / `keyStorePath()` / `rebuildAuthorizedKeys()` / `manageKeys()` に instanceName 追加が必要（manage.ts:100 で取得可能・渡していないだけ）。後方互換は過剰投資不要 | 半日 | ユーザー判断 |
