...
Host ブロックを `injectSshConfig`（HostName/IdentityFile/UserKnownHostsFile
可変化済み）で注入する。docker 非依存 — docker が見つからないマシンでは
main.ts がリモート接続専用モードを案内する。

- **known_hosts はエントリごとに分離**（`~/.ssh/cloopy/known_hosts.d/<名前>`）。
  共有 known_hosts は `refreshKnownHosts` がローカル再起動のたびに丸ごと
  上書きするため、同居させるとリモートのホスト鍵が消える。
...
- **新規 setup のデフォルトは「ローカルのみ」**（127.0.0.1:）。既存 .env
  （変数未設定 = 従来全 IF で使われてきた）の再 setup では「LAN 公開」が
  デフォルト — 無断でリモートアクセスを壊さないため。素の `docker compose`
  利用者も compose デフォルトが空なので挙動不変。
