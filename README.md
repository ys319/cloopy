...
> セキュリティ上の注意: SSH公開範囲のデフォルトは新規セットアップでは「ローカルのみ（127.0.0.1）」です。LAN公開はSSH（公開鍵認証のみ・rootログイン不可）で保護されますが、信頼できないネットワークでは有効にしないでください。インターネットへの直接公開は想定外です（必要ならVPN越しに）。
